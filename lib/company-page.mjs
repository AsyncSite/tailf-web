...
function notFound(canonical) {
  return toResponse(pageShell({
    ...
    status: 404,
    body: `<article class="posting"><h1>회사를 찾을 수 없어요.</h1><p class="meta"><a href="/c/">개발 공고가 열린 회사 모두 보기</a></p></article>${alertBlock()}`,
  }));
}
...
export async function onRequest(context) {
  ...
  if (!/^\d{1,9}$/.test(id) || segs.length > 1) return notFound(SITE + '/c/');
...
${alertBlock('다음 공고는 올라온 날 알려드려요.')}`,
  ...
  const list = open.map((j) => jobRow(j, now, null)).join('\n');
  return toResponse(pageShell({

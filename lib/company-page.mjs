...
import {
  ...
  pageSource, sourceStores, channelStrip, companyStripLead,
} from './seo.mjs';
...
export async function onRequest(context) {
  ...
  // Search visitors get the same band as channel visitors, under their own seo doors.
  const now = new Date();
  ...
  const strip = channelStrip(source || 'seo', alertsHref, companyStripLead(name));

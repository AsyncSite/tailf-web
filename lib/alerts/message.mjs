...
// At most ten postings in one message, whatever the destination.
export const MAX_ITEMS = { email: 10, slack: 10, discord: 10 };

// The role hub the rest of a long list lives on (/r/{slug}/, lib/seo.mjs ROLES).
const ROLE_HUB = { backend: 'backend', fullstack: 'backend', frontend: 'frontend', mobile: 'mobile', dataAi: 'data', cloudInfra: 'devops', security: 'security', qa: 'qa' };

export function moreUrl(conditions) {
  const roles = (conditions && conditions.roles) || [];
  const slugs = [...new Set(roles.map((r) => ROLE_HUB[r]).filter(Boolean))];
  return SITE + '/r/' + (slugs.length === 1 ? slugs[0] + '/' : '');
}
...
export function slackMessage({ matches, conditions, manage }) {
  ...
  if (rest > 0) lines.push('<' + moreUrl(conditions) + '|' + slackEsc(restLine(rest)) + '>');
...
export function discordMessage({ matches, conditions, manage }) {
  ...
  if (rest > 0) lines.push('[' + discordEsc(restLine(rest)) + '](<' + moreUrl(conditions) + '>)');
...
${rest > 0 ? `<p style="margin:10px 0 0;font-size:14px;"><a href="${esc(moreUrl(conditions))}" style="color:#2E7D0F;">${esc(restLine(rest))}</a></p>` : ''}`;
  ...
  const text = [
    ...
    rest > 0 ? '\n' + restLine(rest) + ': ' + moreUrl(conditions) : '',
    '',
    '조건 바꾸기: ' + manage,

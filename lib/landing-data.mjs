...
//   - newest first; a list longer than MAX_PAGES is cut to its newest MAX_PAGES
//     pages instead of hiding the section (2026-09-24: inactive rows keep growing
//     and supply expansion would pass the old 1,500-row guard; the backtest only
//     counts the last 30 days, which sit in the first pages)
...
export const MAX_ROWS = 2000;   // MAX_PAGES * PAGE_SIZE: newest rows kept, older ones dropped
...
export function rowsUrl(page) {
  return API + '?jobFamily=ENGINEERING&page=' + page + '&size=' + PAGE_SIZE + '&includeInactive=true&sortBy=postedAt&sortDirection=DESC';
}

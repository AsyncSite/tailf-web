...
// A post that talks about one company or one posting links to that page with the
// channel as a fixed path segment: /c/{id}/from/{source}/, /p/{id}/from/{source}/.
// Same page, same canonical; the doors on it (/go/…/{source}/, /alerts/from/{source}/)
// keep the channel so Web Analytics counts the whole path per channel from requestPath.
// Every name here has a /go/ page and an /alerts/from/ rewrite (_redirects).
export const PAGE_SOURCES = new Set([
  'threads', 'youtube', 'newsletter', 'lounge', 'cohort', 'community',
  'geeknews', 'okky', 'disquiet', 'velog', 'discord',
]);

/** segs[at..] as a channel tag: undefined when there is none, null when it is not a channel. */
export function pageSource(segs, at) {
  if (segs.length === at) return undefined;
  if (segs.length !== at + 2 || segs[at] !== 'from') return null;
  return PAGE_SOURCES.has(segs[at + 1]) ? segs[at + 1] : null;
}

export function sourceStores(source) {
  return { appstore: '/go/appstore/' + source + '/', play: '/go/play/' + source + '/' };
}

/**
 * A reader lands on a long list or a long posting; the alert block sits after
 * all of it (8,600px down a 70-row company page on a phone). This strip puts the
 * same two doors under the heading, on the reader's own channel (seo for search).
 */
export function channelStrip(source, alertsHref, lead) {
  const stores = sourceStores(source);
  return `<p class="cta-row channel-strip">
  <a class="btn" href="${stores.appstore}" data-install="appstore" data-keep-label>새 공고 알림 받기</a>
  <a class="web-alerts" href="${esc(alertsHref)}">앱 없이 이메일·슬랙으로 받기</a>
</p>
<p class="strip-note">${esc(lead ? lead + ' ' + STRIP_NOTE : STRIP_NOTE)}</p>`;
}

// What the button gives, in the first screen next to it (dev-jobs-143): a reader from a
// post about one company came for that company, and 「새 공고 알림」 alone does not say
// whose postings or how they are picked.
export const STRIP_NOTE = '직무와 경력을 한 번 넣어두면 맞는 새 공고가 올라온 날 알려 드려요. 로그인 없이 써요.';

/** The company page's lead for the strip note: that company is among the pages we read. */
export function companyStripLead(name) {
  return name + ' 채용 페이지도 저희가 매일 보고 있어요.';
}
...
<style>
  ...
  .channel-strip { margin-top: 14px; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 16px; }
  .channel-strip .btn { min-height: 44px; font-size: 15px; }
  .channel-strip .web-alerts { font-size: 14.5px; color: var(--acc-ink); }
  .strip-note { margin-top: 8px; font-size: 14px; line-height: 1.5; color: var(--ink-2); }
 ...
 */
export function alertBlock(lead, alertsHref = '/alerts/from/seo/', stores = null) {
  return `<section class="alert">
  <p class="eyebrow">이런 공고를 매일 찾지 않아도 되게</p>
  <h2>${esc(lead || POSITIONING)}</h2>
  <p class="block-sub">${esc(lead ? POSITIONING + ' 직무와 경력, 쓰는 기술을 한 번 넣어두면 돼요.' : POSITIONING_SUPPORT)} 로그인 없이 쓰고, 조건은 기본으로 폰 안에서 맞춰 봐요.</p>
  <p class="cta-row">
    <a class="btn" href="${(stores && stores.appstore) || STORE_SEO}" data-install="appstore" data-keep-label>새 공고 알림 받기</a>
    <a class="btn install-pending" href="${(stores && stores.play) || PLAY_SEO}" data-install="play">Google Play</a>
  </p>
...
export function jobRow(job, now, companyId, source) {
  const title = titleWithoutCompany(job.title, job.company);
  ...
  return `<li><a class="t" href="/p/${esc(job.id)}/${source ? 'from/' + source + '/' : ''}">${esc(title)}</a><p class="m">${bits.join(' · ')}</p></li>`;
}

...
import {
  ...
  POSITIONING, POSITIONING_SUPPORT,
} from './seo.mjs';
...
export function alertsHrefFor(job) {
  const roles = job ? [...jobRoleFocuses(job)] : [];
  return '/alerts/from/posting/' + (roles.length ? '?role=' + roles.join(',') : '');
}
...
function whyBlock(job) {
  ...
  <p class="block-sub">${esc(POSITIONING_SUPPORT)} 넣어둔 건 그 폰 안에만 있고 로그인도 없어요.</p>
  <p class="cta-row">
...
function relatedLinks(job, companyId, hubs) {
  const links = [];
  if (companyId && job.company) links.push('<a href="/c/' + esc(companyId) + '/">' + esc(job.company) + ' 개발 공고 모두 보기</a>');
...
export async function onRequest(context) {
  ...
  const share = !path.endsWith('/');
  const now = new Date();
  const canonical = SITE + '/p/' + (/^\d{1,12}$/.test(id) ? id + '/' : '');

  if (!/^\d{1,12}$/.test(id) || segs.length > 1) {
...
${relatedLinks(job, companyId, hubs)}
${alertBlock('다음 공고는 올라온 날 알려드려요.', alertsHrefFor(job))}`;
    return toResponse(pageShell({
...
${postingArticle(job, now, { share: false, companyId, expired })}
${relatedLinks(job, companyId, hubs)}
${alertBlock(undefined, alertsHrefFor(job))}`,
  }));
}

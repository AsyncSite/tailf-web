...
import {
  ...
  buildJobPostingLd, breadcrumbLd, ldScript, pageShell, toResponse, alertBlock,
} from './seo.mjs';
...
function errorPage({ title, head, meta, canonical, status, share }) {
  return toResponse(pageShell({
    ...
    body: `<article class="posting"><h1>${esc(head)}</h1>${meta ? '<p class="meta">' + esc(meta) + '</p>' : ''}</article>${share ? whyBlock() : alertBlock()}`,
  }));
}
